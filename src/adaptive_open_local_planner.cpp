...
// Constructor
AdaptiveOpenLocalPlanner::AdaptiveOpenLocalPlanner() : tf_listener(tf_buffer)
{
    ...
    // Subscribe & Advertise
    odom_sub = nh.subscribe(odom_topic_, 1, &AdaptiveOpenLocalPlanner::odomCallback, this);
    obstacles_sub = nh.subscribe(obstacles_topic_, 1, &AdaptiveOpenLocalPlanner::obstaclesCallback, this);
    global_path_sub = nh.subscribe(global_path_topic_, 1, &AdaptiveOpenLocalPlanner::globalPathCallback, this);
    ...
    // timer
    timer = nh.createTimer(ros::Duration(1.0 / planning_frequency_), &AdaptiveOpenLocalPlanner::mainTimerCallback, this);
...
void AdaptiveOpenLocalPlanner::mainTimerCallback(const ros::TimerEvent &timer_event)
{
    if (!b_global_path)
    {
...
void AdaptiveOpenLocalPlanner::odomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg)
{
...
void AdaptiveOpenLocalPlanner::obstaclesCallback(const obstacle_detector::Obstacles::ConstPtr &obstacle_msg)
{
    b_obstacles = true;
    ...
    for (int i = 0; i < obstacle_msg->circles.size(); i++)
    {
        ...
        wp.left_width = obstacle_msg->circles[i].radius;
        wp.right_width = obstacle_msg->circles[i].true_radius;
...
void AdaptiveOpenLocalPlanner::globalPathCallback(const adaptive_open_local_planner::WaypointArray::ConstPtr &global_path_msg)
{
    b_global_path = true;
...
void AdaptiveOpenLocalPlanner::publishCmdVel()
{
...
void AdaptiveOpenLocalPlanner::extractGlobalPathSection(std::vector<Waypoint> &extracted_path)
{
...
void AdaptiveOpenLocalPlanner::generateRollOuts(const std::vector<Waypoint> &path, std::vector<std::vector<Waypoint>> &roll_outs)
{
...
PathCost AdaptiveOpenLocalPlanner::doOneStepStatic(const std::vector<std::vector<Waypoint>> &roll_outs, const std::vector<Waypoint> &extracted_path, std::vector<PathCost> &trajectory_costs)
{
